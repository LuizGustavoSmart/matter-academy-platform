-- Novo papel "embaixador": um aluno que, em turmas específicas, também
-- acompanha métricas (presença, entregas, dúvidas) sem poder criar/editar
-- aulas ou atividades. O papel em `profiles` marca a capacidade; o vínculo
-- por turma em `user_turmas.is_embaixador` decide se, NAQUELA turma, a
-- pessoa é acompanhada como embaixador ou como aluno comum.
...
-- Embaixador precisa ler o mesmo conjunto de dados de turma que professor/
-- monitor já leem hoje (user_turmas, profiles de alunos, presença,
-- atividades/envios, dúvidas, progresso) — mas só leitura, nunca escrita.
